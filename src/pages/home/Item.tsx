...
const Item = ({ title, description, image, url }: ItemHomeProps) => {
  return (
    <article className='flex flex-col bg-black border-primary relative max-w-md'>
      ...
      <div className='text-white flex flex-col lg:gap-y-1 pt-2 pb-3 px-3 text-center'>
        <h2 className='text-xl lg:text-3xl font-condensed'>{title}</h2>
        <p className='whitespace-break-spaces text-sm'>{description}</p>
      </div>
    </article>
  )
}
